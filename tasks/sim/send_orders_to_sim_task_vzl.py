...
fh = TimedRotatingFileHandler('C:/tmp/logs/task_vzl.log', when="midnight", interval=1)
...
rows = sim_db_service.get_orders_to_send_to_sim_vzl()
...
for count, row in enumerate(rows):
    try:
        orderID = str(row[0])
        response = backend3_oms_service.send_order_to_sim_vzl(orderID)
        record = {
            "orderId": int(orderID),
            "code": response["code"],
